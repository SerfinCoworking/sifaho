class WelcomeController < ApplicationController
  ...
  def index
    if current_user.sector.present?
      ...
      @expired_lot_stocks = @lot_stocks.joins("INNER JOIN lots ON lot_stocks.lot_id = lots.id").with_status(2).order('lots.expiry_date').limit(10)
      @near_expiry_lots = @lot_stocks.joins("INNER JOIN lots ON lot_stocks.lot_id = lots.id").with_status(1).order('lots.expiry_date').limit(10)
